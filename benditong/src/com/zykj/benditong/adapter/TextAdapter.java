...
public class TextAdapter extends ArrayAdapter<String> {
	...
	public TextAdapter(Context context, List<String> listData, int sId, int nId) {
		...
		mListData = listData;//数据
		selectedDrawble = mContext.getResources().getDrawable(sId);//当前选中对号
		normalDrawbleId = nId;//当前选中背景
		...
		onClickListener = new OnClickListener() {
			...
			public void onClick(View view) {
				...
				if (mOnItemClickListener != null) {
					mOnItemClickListener.onItemClick(selectedPos, mListData == null?mArrayData[selectedPos]:mListData.get(selectedPos));
				}
			}
		};
	}
	...
	public void notifyCategoryList(List<String> names){
		mListData.addAll(names);
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (selectedText != null && selectedText.equals(mString)) {
			...
			view.setTextColor(mContext.getResources().getColor(R.color.theme_color));
		} else {
			view.setBackgroundDrawable(mContext.getResources().getDrawable(normalDrawbleId));//设置未选中状态背景图片
			view.setTextColor(mContext.getResources().getColor(R.color.tab_font_normal));
		}
